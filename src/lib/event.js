...
export default class Event {
    ...
    setServer(eventServer = false, healthcheck = 'healthcheck') {
        if (!eventServer)
            return this.welWeb.eventServer = false;

        if (utils.isString(eventServer))
            eventServer = new providers.HttpProvider(eventServer);

        if (!this.welWeb.isValidProvider(eventServer))
            throw new Error('Invalid event server provided');

        this.welWeb.eventServer = eventServer;
        this.welWeb.eventServer.isConnected = () => this.welWeb.eventServer.request(healthcheck).then(() => true).catch(() => false);
    }
    ...
    getEventsByContractAddress(contractAddress = false, options = {}, callback = false) {
        ...
        if (contractAddress)
            routeParams.push(this.welWeb.address.fromHex(contractAddress));
        ...
        return this.welWeb.eventServer.request(`event/contract/${routeParams.join('/')}?${querystring.stringify(qs)}`).then((data = false) => {
            if (!data)
    ...
    getEventsByTransactionID(transactionID = false, options = {}, callback = false) {
        ...
        return this.welWeb.eventServer.request(`event/transaction/${transactionID}`).then((data = false) => {
            if (!data)
                return callback('Unknown error occurred');
