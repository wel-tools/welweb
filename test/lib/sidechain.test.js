...
describe("WelWeb.sidechain", function () {
    describe('#deposit', function () {
        describe('#depositTrx()', function () {
            const welWeb = welWebBuilder.createInstanceSide();
            it('deposit trx from main chain to side chain', async function () {
                const callValue = 10000000;
                const txID = await welWeb.sidechain.depositTrx(callValue, DEPOSIT_FEE, FEE_LIMIT);
            ...
            it('depositTrx with the defined private key', async function () {
                const callValue = 10000000;
                const options = {};
                const txID = await welWeb.sidechain.depositTrx(callValue, DEPOSIT_FEE, FEE_LIMIT, options, PRIVATE_KEY);
                assert.equal(txID.length, 64);
            });
            ...
            it('depositTrx with permissionId in options object', async function () {
                ...
                const txID = await welWeb.sidechain.depositTrx(callValue, DEPOSIT_FEE, FEE_LIMIT, options);
                assert.equal(txID.length, 64);
            });
            ...
            it('should throw if an invalid trx number is passed', async function () {
                await assertThrow(
                    welWeb.sidechain.depositTrx(1000.01, DEPOSIT_FEE, FEE_LIMIT),
            ...
            it('should throw if an invalid fee limit is passed', async function () {
                await assertThrow(
                    welWeb.sidechain.depositTrx(10000, DEPOSIT_FEE, 0),
            ...
            it('should check the balance of mainchain and sidechain after depositTrx', async function() {
                const callValue = 10000000;
                const dataBefore = await welWeb.sidechain.sidechain.trx.getAccount();
                const balanceBefore = dataBefore.balance;
                const txID = await welWeb.sidechain.depositTrx(callValue, DEPOSIT_FEE, FEE_LIMIT);
                assert.equal(txID.length, 64);
                await wait(90);
                const dataAfter = await welWeb.sidechain.sidechain.trx.getAccount();
        ...
        describe('#depositTrc10()', function () {
            ...
            it('deposit trc10 from main chain to side chain', async function () {
                const tokenValue = 10;
                const txID = await welWeb.sidechain.depositTrc10(TOKEN_ID, tokenValue, DEPOSIT_FEE, FEE_LIMIT);
                assert.equal(txID.length, 64);
            });
            ...
            it('depositTrc10 with the defined private key', async function () {
                const tokenValue = 10;
                const options = {};
                const txID = await welWeb.sidechain.depositTrc10(TOKEN_ID, tokenValue, DEPOSIT_FEE, FEE_LIMIT, options, PRIVATE_KEY);
                assert.equal(txID.length, 64);
            });
            ...
            it('depositTrc10 with permissionId in options object', async function () {
                const tokenValue = 10;
                const options = { permissionId: 0 };
                const txID = await welWeb.sidechain.depositTrc10(TOKEN_ID, tokenValue, DEPOSIT_FEE, FEE_LIMIT, options);
                assert.equal(txID.length, 64);
            });
            ...
            it('should throw if an invalid token id is passed', async function () {
                const tokenId = -10;
                await assertThrow(
                    welWeb.sidechain.depositTrc10(tokenId, 100, DEPOSIT_FEE, FEE_LIMIT),
            ...
            it('should throw if an invalid token value is passed', async function () {
                ...
                await assertThrow(
                    welWeb.sidechain.depositTrc10(TOKEN_ID, tokenValue, DEPOSIT_FEE, 1000000),
            ...
            it('should throw if an invalid fee limit is passed', async function () {
                const feeLimit = -1
                await assertThrow(
                    welWeb.sidechain.depositTrc10(TOKEN_ID, 100, DEPOSIT_FEE, feeLimit),
                    'Invalid feeLimit provided'
                );
            });
        ...
        describe('#depositTrc20', function () {
            ...
            it('deposit trc20 from main chain to side chain', async function () {
                const num = 100;
                const txID = await welWeb.sidechain.depositTrc20(num, DEPOSIT_FEE, FEE_LIMIT, CONTRACT_ADDRESS20);
                assert.equal(txID.length, 64);
            });
            ...
            it('depositTrc20 with the defined private key', async function () {
                const num = 100;
                const options = {};
                const txID = await welWeb.sidechain.depositTrc20(num, DEPOSIT_FEE, FEE_LIMIT, CONTRACT_ADDRESS20, options, PRIVATE_KEY);
                assert.equal(txID.length, 64);
            });
            ...
            it('depositTrc20 with permissionId in options object', async function () {
                const num = 100;
                const options = { permissionId: 0 };
                const txID = await welWeb.sidechain.depositTrc20(num, DEPOSIT_FEE, FEE_LIMIT, CONTRACT_ADDRESS20, options);
                assert.equal(txID.length, 64);
            });
            ...
            it('should throw if an invalid num is passed', async function () {
                const num = 100.01;
                await assertThrow(
                    welWeb.sidechain.depositTrc20(num, DEPOSIT_FEE, FEE_LIMIT, CONTRACT_ADDRESS20),
            ...
            it('should throw if an invalid fee limit is passed', async function () {
                ...
                const feeLimit = -1;
                await assertThrow(
                    welWeb.sidechain.depositTrc20(num, DEPOSIT_FEE, feeLimit, CONTRACT_ADDRESS20),
                    'Invalid feeLimit provided'
                );
            });
            ...
            it('should throw if an invalid contract address is passed', async function () {
                await assertThrow(
                    welWeb.sidechain.depositTrc20(100, DEPOSIT_FEE, FEE_LIMIT, 'aaaaaaaaaa'),
            ...
            it('should check the trc20 balance after depositTrc20', async function () {
                ...
                // const mappingResult = await welWeb.sidechain.mappingTrc20(HASH20, MAPPING_FEE, FEE_LIMIT,  options);
                // check the trc20 balance of mainchain before deposit
                const contractInstance = await welWeb.contract().at(CONTRACT_ADDRESS20);
                const address = welWeb.address.fromPrivateKey(PRIVATE_KEY);
                const dataBefore = await contractInstance.balanceOf(address).call();
                const balanceBefore = parseInt(dataBefore._hex, 16);

                // approve trc20
                // const approveResult = await welWeb.sidechain.approveTrc20(100000, FEE_LIMIT, CONTRACT_ADDRESS20);
        ...
        describe('#depositTrc721', function () {
            ...
            it('deposit trc721 from main chain to side chain', async function () {
                const txID = await welWeb.sidechain.depositTrc721(TRC721_ID, DEPOSIT_FEE, FEE_LIMIT, CONTRACT_ADDRESS721);
                assert.equal(txID.length, 64);
            });
    ...
    describe('#mappingTrc', function () {
        ...
        it('mappingTrc20', async function () {
            const txID = await welWeb.sidechain.mappingTrc20(HASH20, MAPPING_FEE, FEE_LIMIT);
            assert.equal(txID.length, 64);
        });

        it('mappingTrc20 with the defined private key', async function () {
            const options = {};
            const txID = await welWeb.sidechain.mappingTrc20(HASH20, MAPPING_FEE, FEE_LIMIT, options, PRIVATE_KEY);
            assert.equal(txID.length, 64);
        });

        it('mappingTrc20 with permissionId in options object', async function () {
            const options = { permissionId: 0 };
            const txID = await welWeb.sidechain.mappingTrc20(HASH20, MAPPING_FEE, FEE_LIMIT, options);
            assert.equal(txID.length, 64);
        });
        ...
        it('should throw if an invalid trxHash', async function () {
            const trxHash = '';
            await assertThrow(
                welWeb.sidechain.mappingTrc20(trxHash, MAPPING_FEE, FEE_LIMIT),
        ...
        it('should throw if an invalid fee limit is passed', async function () {
            const feeLimit = -1;
            await assertThrow(
                welWeb.sidechain.mappingTrc20(HASH20, MAPPING_FEE, feeLimit),
                'Invalid feeLimit provided'
            );
        });

        it('check the transaction result after mapping TRC20', async function () {
            const mappingResult = await welWeb.sidechain.mappingTrc20(HASH20, MAPPING_FEE, FEE_LIMIT);
            while(true) {
                let checkResult = await welWeb.trx.getTransactionInfo(mappingResult);
        ...
        it('should get the mapping address after mappingTrc20', async function() {
            const sideGatawayInstance = await welWeb.sidechain.sidechain.contract().at(SIDE_CHAIN.sideOptions.sideGatewayAddress);
            const result = await sideGatawayInstance.mainToSideContractMap(CONTRACT_ADDRESS20).call();
            assert.isTrue(welWeb.isAddress(result));
        });

        it('mappingTrc721', async function () {
            const txID = await welWeb.sidechain.mappingTrc721(HASH721, MAPPING_FEE, FEE_LIMIT);
            assert.equal(txID.length, 64);
        });

        it('check the transaction result after mapping TRC721', async function () {
            const mappingResult = await welWeb.sidechain.mappingTrc721(HASH721, MAPPING_FEE, FEE_LIMIT);
            while(true) {
                let checkResult = await welWeb.trx.getTransactionInfo(mappingResult);
                if (checkResult && checkResult.result) {
                    break;
    ...
    describe('#withdraw', function () {
        describe('#withdrawTrx()', function () {
            ...
            it('withdraw trx from side chain to main chain', async function () {
                const txID = await welWeb.sidechain.withdrawTrx(10000000, WITHDRAW_FEE, 10000000);
                assert.equal(txID.length, 64);
            });
            ...
            it('withdrawTrx with the defined private key', async function () {
                const callValue = 10000000;
                const options = {};
                const txID = await welWeb.sidechain.withdrawTrx(callValue, WITHDRAW_FEE, FEE_LIMIT, options, PRIVATE_KEY);
                assert.equal(txID.length, 64);
            });
            ...
            it('withdrawTrx with permissionId in options object', async function () {
                const callValue = 10000000;
                const options = { permissionId: 0 };
                const txID = await welWeb.sidechain.withdrawTrx(callValue, WITHDRAW_FEE, FEE_LIMIT, options);
                assert.equal(txID.length, 64);
            });
            ...
            it('should throw if an invalid trx number is passed', async function () {
                await assertThrow(
                    welWeb.sidechain.withdrawTrx(1000.01, WITHDRAW_FEE, FEE_LIMIT),
                    'Invalid callValue provided'
                );
            });
            ...
            it('should throw if an invalid fee limit is passed', async function () {
                await assertThrow(
                    welWeb.sidechain.withdrawTrx(10000, WITHDRAW_FEE, 0),
                    'Invalid feeLimit provided'
                );
            });
            ...
            it('should check the balance of mainchain and sidechain after withdrawTrx', async function() {
                const callValue = 10000000;
                const dataBefore = await welWeb.trx.getAccount();
                const balanceBefore = dataBefore.balance;
                const txID = await welWeb.sidechain.withdrawTrx(callValue, WITHDRAW_FEE, FEE_LIMIT);
                await wait(90);
                const dataAfter = await welWeb.trx.getAccount();
                const balanceAfter = dataAfter.balance;
                assert.equal(balanceBefore + callValue, balanceAfter);
            });
        });
        ...
        describe('#withdrawTrc10()', function () {
            ...
            it('withdraw trc10 from side chain to main chain', async function () {
                const tokenValue = 10;
                const txID = await welWeb.sidechain.withdrawTrc10(TOKEN_ID, tokenValue, WITHDRAW_FEE, FEE_LIMIT);
                assert.equal(txID.length, 64);
            });
            ...
            it('withdrawTrc10 with the defined private key', async function () {
                const tokenValue = 10;
                const options = {};
                const txID = await welWeb.sidechain.withdrawTrc10(TOKEN_ID, tokenValue, WITHDRAW_FEE, FEE_LIMIT, options, PRIVATE_KEY);
                assert.equal(txID.length, 64);
            });
            ...
            it('withdrawTrc10 with permissionId in options object', async function () {
                const tokenValue = 10;
                const options = { permissionId: 0 };
                const txID = await welWeb.sidechain.withdrawTrc10(TOKEN_ID, tokenValue, WITHDRAW_FEE, FEE_LIMIT, options);
                assert.equal(txID.length, 64);
            });
            ...
            it('should throw if an invalid token id is passed', async function () {
                const tokenId = -10;
                await assertThrow(
                    welWeb.sidechain.withdrawTrc10(tokenId, 100, WITHDRAW_FEE, 1000000),
                    'Invalid tokenId provided'
                )
            });
            ...
            it('should throw if an invalid token value is passed', async function () {
                ...
                await assertThrow(
                    welWeb.sidechain.withdrawTrc10(TOKEN_ID, tokenValue, WITHDRAW_FEE, FEE_LIMIT),
                    'Invalid tokenValue provided'
                );
            });
            ...
            it('should throw if an invalid fee limit is passed', async function () {
                const feeLimit = -1;
                await assertThrow(
                    welWeb.sidechain.withdrawTrc10(TOKEN_ID, 100, WITHDRAW_FEE, feeLimit),
                    'Invalid feeLimit provided'
                );
            });
            ...
            it('should check the TRC10 balance of mainchain and sidechain after withdrawTrc10', async function() {
                ...
                const dataBefore = await welWeb.trx.getAccount();
                const balanceBefore = dataBefore.assetV2.filter((item)=> item.key == TOKEN_ID)[0].value;
                const txID = await welWeb.sidechain.withdrawTrc10(TOKEN_ID, tokenValue, DEPOSIT_FEE, FEE_LIMIT);
                assert.equal(txID.length, 64);
                await wait(90);
                const dataAfter = await welWeb.trx.getAccount();
                const balanceAfter = dataAfter.assetV2.filter((item)=> item.key == TOKEN_ID)[0].value;;
        ...
        describe('#withdrawTrc', function () {
            describe('#withdrawTrc20', function () {
                ...
                it('withdraw trc20 from side chain to main chain', async function () {
                    const num = 10;
                    const txID = await welWeb.sidechain.withdrawTrc20(num, WITHDRAW_FEE, FEE_LIMIT, ADDRESS20_MAPPING);
                    assert.equal(txID.length, 64);
                });
                ...
                it('withdrawTrc20 with the defined private key', async function () {
                    const num = 10;
                    const options = {};
                    const txID = await welWeb.sidechain.withdrawTrc20(num, WITHDRAW_FEE, FEE_LIMIT, ADDRESS20_MAPPING, options, PRIVATE_KEY);
                    assert.equal(txID.length, 64);
                });
                ...
                it('withdrawTrc20 with permissionId in options object', async function () {
                    const num = 10;
                    const options = { permissionId: 0 };
                    const txID = await welWeb.sidechain.withdrawTrc20(num, WITHDRAW_FEE, FEE_LIMIT, ADDRESS20_MAPPING, options);
                    assert.equal(txID.length, 64);
                });
                ...
                it('should throw if an invalid num is passed', async function () {
                    const num = 10.01;
                    await assertThrow(
                        welWeb.sidechain.withdrawTrc20(num, WITHDRAW_FEE, FEE_LIMIT, ADDRESS20_MAPPING),
                ...
                it('should throw if an invalid fee limit is passed', async function () {
                    ...
                    assert.equal(txID.length, 64);
                    // await assertThrow(
                    //     welWeb.sidechain.withdrawTrc20(100, WITHDRAW_FEE, feeLimit, ADDRESS20_MAPPING),
                    //     'Invalid feeLimit provided'
                    // );
                });
                ...
                it('should throw if an invalid contract address is passed', async function () {
                    await assertThrow(
                        welWeb.sidechain.withdrawTrc20(100, WITHDRAW_FEE, FEE_LIMIT, 'aaaaaaaaaa'),
                        'Invalid contractAddress address provided'
                    );
                });
            ...
            describe('#withdrawTrc721', async function () {
                ...
                it('withdraw trc721 from side chain to main chain', async function () {
                    const txID = await welWeb.sidechain.withdrawTrc721(TRC721_ID, WITHDRAW_FEE, FEE_LIMIT, ADDRESS20_MAPPING);
                    assert.equal(txID.length, 64);
                });
            });
        });
    ...
    describe('#injectFund', function() {
        it('excute injectFund', async function() {
            const welWeb = welWebBuilder.createInstanceSide();
            const txID = await welWeb.sidechain.injectFund(1000000, FEE_LIMIT);
            assert.equal(txID.length, 64);
        });
    });
    ...
    describe('#retry', function() {
        it('retry mapping', async function() {
            const welWeb = welWebBuilder.createInstanceSide();
            const txID = await welWeb.sidechain.retryMapping(NONCE, RETRY_MAPPING_FEE, FEE_LIMIT);
            assert.equal(txID.length, 64);
        });

        it('retry deposit', async function() {
            const welWeb = welWebBuilder.createInstanceSide();
            const txID = await welWeb.sidechain.retryDeposit(NONCE, RETRY_DEPOSIT_FEE, FEE_LIMIT);
            assert.equal(txID.length, 64);
        });

        it('retry withdraw', async function() {
            const welWeb = welWebBuilder.createInstanceSide();
            const txID = await welWeb.sidechain.retryWithdraw(NONCE, RETRY_WITHDRAW_FEE, FEE_LIMIT);
            assert.equal(txID.length, 64);
        });
    });
