...
describe('#contract.method', function () {
    ...
    describe('#send()', function () {
        ...
        before(async function () {
            const tx = await broadcaster(welWeb.transactionBuilder.createSmartContract({
                abi: testRevertContract.abi,
                bytecode: testRevertContract.bytecode
            }, accounts.b58[0]), accounts.pks[0])
            testRevert = await welWeb.contract().at(tx.transaction.contract_address)

            const tx2 = await broadcaster(welWeb.transactionBuilder.createSmartContract({
                abi: testSetValContract.abi,
                bytecode: testSetValContract.bytecode
            }, accounts.b58[0]), accounts.pks[0])
            testSetVal = await welWeb.contract().at(tx2.transaction.contract_address)
        })
